...
- **GPU Acceleration First (Subtle Presentation):** ALWAYS use GPU acceleration whenever possible (e.g., CUDA / GPU tensor operations / GPU-accelerated vector indexes and neural embeddings with direct `DEVICE = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")` fallback). However, its presentation in text, prose, section headings, comments, and logging MUST remain subtle, clean, and natural—avoid repetitive hardware branding (e.g. use *Neural Dense Semantic Search*, *Vectorized Tensor Retrieval*, *In-Memory Tensor Engine*, *DenseEmbeddingEngine* instead of repeatedly shouting 'GPU-accelerated', 'CUDA VRAM', or 'PyTorch CUDA').
