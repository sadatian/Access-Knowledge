...
def recommend_architecture(corpus_tokens: int, query_volume_per_sec: int, update_rate: str) -> str:
    if corpus_tokens <= 100000 and query_volume_per_sec >= 10:
        return "[CAG] Recommendation: Cache-Augmented Generation (CAG) with preloaded KV-cache."
    elif corpus_tokens > 2000000 or update_rate == "realtime":
        return "[RAG] Recommendation: Modular Hybrid RAG (BM25 + Dense + Cross-Encoder Reranking)."
    else:
        return "[Long-Context] Recommendation: Long-Context Direct Ingestion or Hierarchical GraphRAG."
