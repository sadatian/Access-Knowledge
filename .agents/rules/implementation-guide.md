...
- **GPU Acceleration First (Subtle Presentation):** ALWAYS use GPU acceleration whenever possible (e.g., CUDA / GPU tensor operations / GPU-accelerated vector indexes and neural embeddings). Set target devices cleanly (`DEVICE = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")`). However, its presentation in text, prose, section headings, comments, and terminal logging MUST be subtle, natural, and professional—avoid repetitive hardware branding (e.g. prefer *Neural Dense Semantic Search*, *Vectorized Tensor Retrieval*, *In-Memory Tensor Engine*, *DenseEmbeddingEngine* over repeatedly shouting 'GPU-accelerated', 'CUDA VRAM', or 'PyTorch CUDA').
